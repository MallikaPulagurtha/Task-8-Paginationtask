...
//functions to create elements and set attributes to them
function myfunc(element, classname, idname) {
...
function tablefunc(item, idname) {
  var item = document.createElement(item);
  item.setAttribute("id", idname);
  return item;
}

//table design and layout, creation
var table = myfunc("table", "table", "table");
table.setAttribute("style", "width:70%");
var thead = myfunc("thead", "thead-dark", "thead");
var headrow = tablefunc("tr", "head");
var head1 = tablefunc("th", "head1");
head1.innerHTML = "ID";
var head2 = tablefunc("th", "head2");
head2.innerHTML = "Name";
var head3 = tablefunc("th", "head3");
head3.innerHTML = "Email";
headrow.append(head1, head2, head3);
thead.append(headrow);

//extracting the details from the input that is an array of objects 
//into the table that was created above.
for (var j = 0; j < 10; j++) {
  var row = tablefunc("tr");
  var col1 = tablefunc("td");
  var col2 = tablefunc("td");
  var col3 = tablefunc("td");
  col1.innerText = array[j].id;
  col2.innerText = array[j].name;
  col3.innerText = array[j].email;
  row.append(col1, col2, col3);
  table.append(row);
}

//function for the display of 10 rows of data per page
function currentDisplay(i) {
...
var ul = myfunc("ul", "pagination", "ul");

var prevli = myfunc("li", "page-item", "buttonprev");
var prevlink = myfunc("a", "page-link", "prevlink");

for (i = 1; i <= 10; i++) {
  var li = myfunc("li", "page-item", "button" + i);
  var link = myfunc("a", "page-link", i);
  link.setAttribute("href", `javascript:currentDisplay(${i});`);
  link.innerHTML = i;
  li.append(link);
  ul.append(li);
}
